...
class VizSeqTrainer():
    ...
    def metric_func(self):
        if self.metric:
            ...
            evaluate = lambda x, y: 2 * (x * y) / (x + y + 1e-20)
        return evaluate
    ...
    def validate(
        ...
        net.eval()
        criterion = self.loss_func()
        metric_criterion = self.metric_func()
        loss = 0.0
        metric = 0.0
        ...
        with torch.no_grad():
            for i, batch in tqdm(enumerate(valloader), total=len(valloader)):
                ...
                predictions = torch.nn.functional.one_hot(predictions, num_classes=labels.shape[-1])
                metric += metric_criterion(predictions, labels).mean().item()
