...
def generate_manuscript_from_corpus(
        ...
        scripture_corruption_prototypes_cache=cache_images(
            "./assets/scripture_corruption_prototypes/"),
        on_token_paste=None,
):
    ...
    def sample_word_spacing():
        return np.random.randint(int(word_spacing * (1 - word_spacing_variance)),
                                 int(word_spacing * (1 + word_spacing_variance)))
    ...
    def sample_prototype(cache=scripture_prototypes_cache):
        ...
        prototype = transform.rotate(prototype, angle, resize=False, center=None, order=1,
    ...
    while positions:
        ...
        overlap = np.where(
            manuscript[i_paste_range[0]:i_paste_range[1], j_paste_range[0]:j_paste_range[1]] * img,
            1, 0).astype(np.uint8)
        # zero out overlap
        img *= (1 - overlap)
        manuscript[i_paste_range[0]:i_paste_range[1],
        j_paste_range[0]:j_paste_range[1]] += img.astype(np.uint8)
        # Proceed to the next character on the same line
        positions.append((i, j + direction * (font_width + char_space)))

        ## EVENTS
        if on_token_paste:
            on_token_paste({"token": char, "position": (i, j), "font_size": font_size,
                            "char_spacing": char_space, "line_shift": line_shift,
